...
import { Check, X, Loader2, TrendingUp, TrendingDown, Clock, Lock } from 'lucide-react';
...
export const PredictionCard: React.FC<PredictionCardProps> = ({ market, huddleId }) => {
  ...
  // Fetch user's existing bet and community stats
  useEffect(() => {
  ...
  }, [user, placing, userBet, isLocked, isResolved, market.id, huddleId]);

  if (loadingBet) {
    return (
      <Card className="p-3 bg-muted/30 border-border/50 animate-pulse">
        <div className="h-16" />
      </Card>
    );
  }
    ...
    <Card className={cn(
      ...
      {isResolved && (
        <div className="space-y-2">
          ...
          {userBet && (
            <div className={cn(
              "text-sm font-medium",
              userBet.won ? "text-green-400" : "text-red-400"
            )}>
              {userBet.won 
                ? `🏆 You won ${(userBet.chips_won || 100) - userBet.chips_risked}¢!`
                : `❌ You lost ${userBet.chips_risked}¢`
              }
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2">
          ...
          >
            {placing ? <Loader2 className="h-4 w-4 animate-spin" /> : (
              <>YES <span className="ml-1 opacity-80">{yesCost}¢</span></>
            )}
          </Button>
          <Button
          ...
          >
            {placing ? <Loader2 className="h-4 w-4 animate-spin" /> : (
              <>NO <span className="ml-1 opacity-80">{noCost}¢</span></>
            )}
          </Button>
        </div>
      )}
      ...
      {!isResolved && market.event_start_time && (
        <div className="flex items-center gap-1 mt-2 text-[10px] text-muted-foreground">
          ...
          {isPast(new Date(market.event_start_time))
            ? 'Game in progress'
