...
import { ArrowLeft, TrendingUp, TrendingDown, Trophy, Clock, Check, X, Target, Coins, BarChart3, Settings } from 'lucide-react';
...
export default function Ledger() {
  ...
  const { user } = useAuth();
  const { portfolio, loading: portfolioLoading, winRate, profit, isPremium, isOutOfChips, minimumChips, startingChips } = usePortfolio();
                ...
                {closedBets.map(bet => (
                  <Card key={bet.id} className={cn("p-4 border-2",
                    ...
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={cn("text-xs px-2 py-0.5 rounded-full font-bold flex items-center gap-1 ml-2 shrink-0",
