...
-- ---------------------------------------------------------------------------
-- Confirm it worked. Expected after running:
--   * kalshi-post-predictions-hourly-game-hours  -> GONE
--   * fade-post-props-hourly                     -> present, '45 15-23,0-3 * * *'
--   * kalshi-sync-markets / kalshi-settle        -> still present (fade needs them)
-- ---------------------------------------------------------------------------
SELECT jobname, schedule, active
FROM cron.job
WHERE jobname LIKE 'kalshi%' OR jobname LIKE 'fade%'
ORDER BY jobname;
